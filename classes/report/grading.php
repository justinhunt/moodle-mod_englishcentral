...
class grading extends basereport {
        ...
        switch ($field) {
            ...
            case 'username':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php',
                            array('action' => 'gradingbyuser', 'n' => $record->ecid, 'userid' => $record->userid));
            ...
            case 'gradenow':
                if ($withlinks) {
                    ...
                    $link = new \moodle_url(constants::M_URL . '/grading.php',
                            array('action' => 'gradenow', 'n' => $record->ecid, 'attemptid' => $record->id));
            ...
            case 'aigradenow':
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php',
                            array('action' => 'aigradenow', 'n' => $record->ecid, 'attemptid' => $record->id));
            ...
            case 'deletenow':
                if ($withlinks) {
                    $url = new \moodle_url(constants::M_URL . '/manageattempts.php',
                            array('action' => 'delete', 'n' => $record->ecid, 'attemptid' => $record->id,
                                    'source' => $this->report));
    ...
    public function process_raw_data($formdata) {
        ...
        $moduleinstance = $DB->get_record(constants::M_TABLE, array('id' => $formdata->ecid));
        ...
        if($formdata->groupid > 0){
            ...
            $human_sql = "SELECT tu.*, false as fulltranscript  FROM {" . constants::M_USERTABLE . "} tu " .
                    ...
                    " WHERE gm.groupid $groupswhere AND tu.ecid=? AND tu.dontgrade = 0 " .
        ...
        //if we are not doing groups its easier. Show it all
            ...
            $human_sql = "SELECT tu.*, false as fulltranscript  FROM {" . constants::M_USERTABLE .
                    "} tu INNER JOIN {user} u ON tu.userid=u.id WHERE tu.ecid=?  AND tu.dontgrade = 0 " .
                    " ORDER BY u.lastnamephonetic,u.firstnamephonetic,u.lastname,u.firstname,u.middlename,u.alternatename,tu.id DESC";
            ...
            $hybrid_sql =
                    ...
                    constants::M_USERTABLE . "} tu INNER JOIN {user} u ON tu.userid=u.id " .
                    "INNER JOIN {" . constants::M_AITABLE . "} tai ON tai.attemptid=tu.id " .
                    "WHERE tu.ecid=?  AND tu.dontgrade = 0 " .
                    " ORDER BY u.lastnamephonetic,u.firstnamephonetic,u.lastname,u.firstname,u.middlename,u.alternatename,tu.id DESC";
        ...
        if ($alldata) {
            ...
            foreach ($alldata as $thedata) {
                ...
                $thedata->audiourl =
                        \mod_englishcentral\utils::make_audio_URL($thedata->filename, $formdata->modulecontextid, constants::M_COMPONENT,
