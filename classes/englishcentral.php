...
class englishcentral {
  ...
  function build_authorize_token($user){
      ...
      //NB The decrypted encrypted_secret is NOT the same as the consumer_secret ...?
      //why?
      //echo ($this->encrypted_secret);
      //echo '<br>';
     ...
     // print_r($payload);
    //  echo '<br>';
      $jwt = \mod_englishcentral\Firebase\JWT\JWT::encode(
          $payload,
          $consumersecret
      );
  ...
  public function get_auth_header($sdk_token){
      $consumersecret = \mod_englishcentral\Firebase\JWT\JWT::urlsafeB64Decode($this->encrypted_secret);
      $payload = (array)\mod_englishcentral\Firebase\JWT\JWT::decode($sdk_token,$consumersecret,['HS256']);

      $jwt = 'JWT ' . \mod_englishcentral\Firebase\JWT\JWT::encode(
              [
    ...
    public function doPost($jwt, $url, $fields = [], $accept)
    {
        $ch = curl_init();
        curl_setopt( $ch, CURLOPT_URL, $url );
        curl_setopt( $ch, CURLOPT_FOLLOWLOCATION, true );
