...
switch ($action){
	...
	case 'generatedata':
	    $completeattempts = $DB->get_records(constants::M_ATTEMPTSTABLE,
            array('ecid'=>  $moduleinstance->id,'watchcomplete'=>1,'learncomplete'=>1,'speakcomplete'=>1,'chatcomplete'=>1),
                'timecompleted DESC','*',0,1);
        $videos = $DB->get_fieldset(constants::M_VIDEOSTABLE,'id',array('ecid'=>$moduleinstance->id));
